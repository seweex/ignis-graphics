#ifndef IGNIS_INPUT_EVENT_HXX
#define IGNIS_INPUT_EVENT_HXX
...
namespace Ignis::Input
{
    ...
    struct ButtonEvent
    {
        ...
        [[nodiscard]] bool operator ==(ButtonEvent const& event) const noexcept = default;
        [[nodiscard]] bool operator !=(ButtonEvent const& event) const noexcept = default;
    };
    ...
    struct MouseState
    {
        ...
        [[nodiscard]] bool operator ==(MouseState const& event) const noexcept = default;
        [[nodiscard]] bool operator !=(MouseState const& event) const noexcept = default;
    };
}
...
{
    class EventHash final
    {
    public:
        [[nodiscard]] size_t operator() (Input::ButtonEvent const event)
        const noexcept
        {
            uint32_t const eventValue =
                (static_cast <uint16_t> (event.action) << 16) ^
                (static_cast <uint16_t> (event.key));

            return std::hash <uint32_t> {} (eventValue);
        }
    };
}
